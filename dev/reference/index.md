# Package index

## Embed categorical predictors

- [`step_embed()`](https://rmsharp.github.io/embedmit/dev/reference/step_embed.md)
  [`embed_control()`](https://rmsharp.github.io/embedmit/dev/reference/step_embed.md)
  : Encoding Factors into Multiple Columns
- [`step_feature_hash()`](https://rmsharp.github.io/embedmit/dev/reference/step_feature_hash.md)
  **\[deprecated\]** : Dummy Variables Creation via Feature Hashing
- [`step_lencode()`](https://rmsharp.github.io/embedmit/dev/reference/step_lencode.md)
  : Likelihood encoding using analytical formula
- [`step_lencode_bayes()`](https://rmsharp.github.io/embedmit/dev/reference/step_lencode_bayes.md)
  : Supervised Factor Conversions into Linear Functions using Bayesian
  Likelihood Encodings
- [`step_lencode_glm()`](https://rmsharp.github.io/embedmit/dev/reference/step_lencode_glm.md)
  : Supervised Factor Conversions into Linear Functions using Likelihood
  Encodings
- [`step_lencode_mixed()`](https://rmsharp.github.io/embedmit/dev/reference/step_lencode_mixed.md)
  : Supervised Factor Conversions into Linear Functions using Bayesian
  Likelihood Encodings
- [`step_woe()`](https://rmsharp.github.io/embedmit/dev/reference/step_woe.md)
  : Weight of evidence transformation

## Collapse categorical predictors

- [`step_collapse_cart()`](https://rmsharp.github.io/embedmit/dev/reference/step_collapse_cart.md)
  : Supervised Collapsing of Factor Levels
- [`step_collapse_stringdist()`](https://rmsharp.github.io/embedmit/dev/reference/step_collapse_stringdist.md)
  : collapse factor levels using stringdist

## Embed numeric predictors

- [`step_discretize_cart()`](https://rmsharp.github.io/embedmit/dev/reference/step_discretize_cart.md)
  : Discretize numeric variables with CART
- [`step_discretize_xgb()`](https://rmsharp.github.io/embedmit/dev/reference/step_discretize_xgb.md)
  : Discretize numeric variables with XgBoost
- [`step_pca_sparse()`](https://rmsharp.github.io/embedmit/dev/reference/step_pca_sparse.md)
  : Sparse PCA Signal Extraction
- [`step_pca_sparse_bayes()`](https://rmsharp.github.io/embedmit/dev/reference/step_pca_sparse_bayes.md)
  : Sparse Bayesian PCA Signal Extraction
- [`step_pca_truncated()`](https://rmsharp.github.io/embedmit/dev/reference/step_pca_truncated.md)
  : Truncated PCA Signal Extraction
- [`step_umap()`](https://rmsharp.github.io/embedmit/dev/reference/step_umap.md)
  : Supervised and unsupervised uniform manifold approximation and
  projection (UMAP)

## Datasets

- [`solubility`](https://rmsharp.github.io/embedmit/dev/reference/solubility.md)
  : Compound solubility data

## Weight of evidence helpers

- [`add_woe()`](https://rmsharp.github.io/embedmit/dev/reference/add_woe.md)
  : Add WoE in a data frame
- [`woe_table()`](https://rmsharp.github.io/embedmit/dev/reference/woe_table.md)
  : Crosstable with woe between a binary outcome and a predictor
  variable.

## miscellaneous

- [`dictionary()`](https://rmsharp.github.io/embedmit/dev/reference/dictionary.md)
  : Weight of evidence dictionary
